const data = {
    ...
    teammaker: {
        DESCRIPTION: "Eenvoudige Teams maken doe je zo!",
        EXTENDED_DESCRIPTION: "```Wil je op een eerlijke manier groepen maken, dan help ik je even!",
        NO_TEXT: "```Wie doen er mee?```",
        TAG_PERSON: "```Reply to the person that should be included in group```",
        GROUP_CREATED: "```Teams zijn aangemaakt.```"
    },
    cpp: {
    ...
    weather: {
        ...
        WEATHER_DATA:
            "*Temperatuur:* {tempInC} °C\n*Min Temp:* {minTempInC} °C\n*Max Temp:* {maxTempInC} °C\n*Vochtigheid:* {humidity}%\n*Wind:* {windSpeedInkmph} kmph, {degree}°\n*Zonsopgang:* {sunriseTime}\n*Zonsondergang:* {sunsetTime}\n\n\n*{weatherDescription}*\n{cityName} , {country}\n{dateAndTime}",
        CITY_NAME_REQUIRED:
    ...
    pornhub: {
        DESCRIPTION: "Voor je dagelijkse trekken",
        EXTENDED_DESCRIPTION:
            "```Probeer het maar eens, wie weet vind je het wat.```",
        CAPTION_DATA: "*Title:* {titel}\n*Views:* {views}\n*Rate:* {rate}\n*Lengte min:* {length_min}\n*Video:* {video}",
        ERROR_OCCURED:
            "```Woops, cannot process this request. Try again later.```",
        DOWNLOADING: "```Even geduld...```",
        NOT_FOUND: "```Niks gevonden!```"
    },
    imdb: {
        DESCRIPTION: "Opzoek naar info over een film of serie, try this",
        EXTENDED_DESCRIPTION:
            "```Probeer het maar eens, wie weet vind je het wat.```",
        ERROR_OCCURED:
            "```Woops, cannot process this request. Try again later.```",
